...
For Railway, set `DATABASE_PATH` to a persistent volume location such as `/data/big-jims-pickels.sqlite` so request data survives restarts and new deploys. Keep the other values in Railway environment variables instead of committing them to the repository.
